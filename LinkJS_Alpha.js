 ...
 * LinkJS不是构造函数。
 * @name LinkJS
 * @class LinkJS是LinkJSJS框架的全局对象，也是框架内部所有类的全局命名空间。在全局Q未被占用的情况下，也可以使用其缩写Q。
 */
var LinkJS = win.LinkJS = win.LinkJS || 
{
	global: win
};
 ...
 */
LinkJS.merge = function(obj, props, strict)
{
 ...
 */
LinkJS.delegate = function(func, self)
{
 ...
 */
LinkJS.createDOM = function(type, props)
{
...
/**
 * 根据限定名称返回一个命名空间（从global开始）。如：LinkJS.use('LinkJS.test')。
 * @param {String} 指定新的命名空间的名称。如LinkJS.test等。
 * @return {Object} 参数name指定的命名空间对象。
 */
LinkJS.use = function(name)
{
 ...
 */
LinkJS.getElementOffset = function(elem)
{
 ...
 */
LinkJS.createDOMDrawable = function(disObj, imageObj)
{
 ...
 * 角度转弧度常量。
 */
LinkJS.DEG_TO_RAD = Math.PI / 180;

/**
 * 弧度转角度常量。
 */
LinkJS.RAD_TO_DEG = 180 / Math.PI;
 ...
 */
LinkJS.hitTestPoint = function(obj, x, y, usePolyCollision)
{
 ...
 */
LinkJS.hitTestObject = function(obj1, obj2, usePolyCollision)
{
	...
	if(hit && usePolyCollision)
	{
		hit = LinkJS.polygonCollision(b1, b2);
		return hit !== false;
	}
	return hit;
};
 ...
 */
LinkJS.polygonCollision = function(poly1, poly2)
{	
...
/**
 * 返回LinkJS的字符串表示形式。
 * @return {String} LinkJS的字符串表示形式。
 */
LinkJS.toString = function()
{
	return "LinkJS";
};
...
/**
 * 默认的全局namespace为LinkJS或Q（当Q没有被占据的情况下）。
 */
if(win.Q == undefined) win.Q = LinkJS;
if(win.trace == undefined) win.trace = LinkJS.trace;
...
var Matrix = LinkJS.Matrix = function(a, b, c, d, tx, ty)
{
	this.a = a;
	this.b = b;
	this.c = c;
...
var Rectangle = LinkJS.Rectangle = function(x, y, width, height)
{
	this.x = x;
	this.y = y;
	this.width = width;
 ...
 */
var EventManager = LinkJS.EventManager = function()
{
	this.keyState = {};
	this._evtHandlers = {};
};
...
/**
 * 注册LinkJS.Stage事件侦听，使得Stage能够接收和处理指定的事件。
 * @param stage LinkJS.Stage舞台对象。
 ...
 */
var EventDispatcher = LinkJS.EventDispatcher = function()
{
 ...
 * 构造函数.
 * @name Context
 * @class Context是LinkJS框架中显示对象结构的上下文，实现显示对象结构的渲染。此类为抽象类。
 * @param {Object} props 一个对象。包含以下属性：
 * <p>canvas - 渲染上下文所对应的画布。</p>
 */
var Context = LinkJS.Context = function(props)
{	
	if(props.canvas == null) throw "LinkJS.Context Error: canvas is required.";
	
	this.canvas = null;
	LinkJS.merge(this, props);
};
 ...
 */
var CanvasContext = LinkJS.CanvasContext = function(props)
{
	CanvasContext.superClass.constructor.call(this, props);
	this.context = this.canvas.getContext("2d");
};
LinkJS.inherit(CanvasContext, LinkJS.Context);
...
CanvasContext.prototype.draw = function(target)
{
	...
	if(target.mask != null)
	{
		...
		target.mask = null;
		if(target instanceof LinkJS.DisplayObjectContainer)
		{
			//container's children should draw at once in 'source-in' mode.
			var cache = target._cache || LinkJS.cacheObject(target);
	...
	}else if(target instanceof LinkJS.Graphics || target instanceof LinkJS.Text)
	{
		//special drawing
...
CanvasContext.prototype.transform = function(target)
{
	...
	}else
	{
		...
		if(target.rotation%360 != 0) ctx.rotate(target.rotation%360*LinkJS.DEG_TO_RAD);
...
var supportTransform = testElem.style[LinkJS.cssPrefix + "Transform"] != undefined;
var supportTransform3D = testElem.style[LinkJS.cssPrefix + "Perspective"] != undefined;
...
LinkJS.supportTransform = supportTransform;
LinkJS.supportTransform3D = supportTransform3D;
if(!supportTransform)
{
 ...
 */
var DOMContext = LinkJS.DOMContext = function(props)
{
	DOMContext.superClass.constructor.call(this, props);
};
LinkJS.inherit(DOMContext, LinkJS.Context);
...
DOMContext.prototype.draw = function(target)
{
	if(!target._addedToDOM)
	{
		...
		if(parent != null)
		{
			...
			if(parentDOM.parentNode == null && parent instanceof LinkJS.Stage) 
			{
...
{	
	...
	{
		var css = LinkJS.supportTransform3D ? getTransformCSS(target, true) : getTransformCSS(target, false);
		style[transform] = css;
	}
 ...
 * @private
 */
var UIDUtil = LinkJS.UIDUtil = { _counter:0 };
 ...
 */
LinkJS.toggleDebugRect = function(stage)
{
	...
	if(stage.debug)
	{
		stage._render = function(context)
		{
			...
			LinkJS.Stage.superClass._render.call(stage, context);
 ...
 */
LinkJS.cacheObject = function(obj, toImage, type)
{
	var w = obj.width, h = obj.height, mask = obj.mask;
	var canvas = LinkJS.createDOM("canvas", {width:w, height:h});
	var context = new LinkJS.CanvasContext({canvas:canvas});
	obj.mask = null;
	obj.render(context);
	obj.mask = mask;
 ...
 * @private
 */
LinkJS._helpContext = new LinkJS.CanvasContext({canvas:LinkJS.createDOM("canvas")});
 ...
 */
var Timer = LinkJS.Timer = function(interval)
{	
 ...
 */
var ImageLoader = LinkJS.ImageLoader = function(source)
{
	...
	this._images = {};
	this._totalSize = 0;
	this._loadHandler = LinkJS.delegate(this._loadHandler, this);
	
	this._addSource(source);
};
LinkJS.inherit(ImageLoader, LinkJS.EventDispatcher);
 ...
 */
var Tween = LinkJS.Tween = function(target, newProps, params)
{
	this.target = target;
	this.time = 0;
	this.delay = 0;
...
/**
 * 更新所有Tween实例，一般由LinkJS.Timer类自动调用。
 * @static
 */
Tween.step = function()
{
 ...
 * @param loop 指示是否循环播放。
 */
var Audio = LinkJS.Audio = function(src, preload, autoPlay, loop)
{	
	...
	this._loaded = false;
    this._playing = false;
	this._evtHandler = LinkJS.delegate(this._evtHandler, this);
...
LinkJS.inherit(Audio, LinkJS.EventDispatcher);
...
Audio.prototype._evtHandler = function(e)
{
	...
	{
		trace("LinkJS.Audio Error: " + e.target.src);
	}
};
 ...
 */
var Drawable = LinkJS.Drawable = function(drawable, isDOM)
{	
...
Drawable.prototype.get = function(obj, context)
{
	...
	}else
	{
		...
		{
			this.domDrawable = LinkJS.createDOMDrawable(obj, {image:this.rawDrawable});
		}
 ...
 */	
var DisplayObject = LinkJS.DisplayObject = function(props)
{
	this.id = LinkJS.UIDUtil.createUID("DisplayObject");
	...
	LinkJS.merge(this, props, true);
	if(props.mixin) LinkJS.merge(this, props.mixin, false);

	DisplayObject.superClass.constructor.call(this, props);
};
LinkJS.inherit(DisplayObject, LinkJS.EventDispatcher);
...
{ 
	...
	{
		this.drawable = new LinkJS.Drawable(drawable);
...
{
	return LinkJS.hitTestPoint(this, x, y, usePolyCollision);
};
...
{
	return LinkJS.hitTestObject(this, object, usePolyCollision);
};
...
DisplayObject.prototype.globalToLocal = function(x, y) 
{
	...
	cm = new LinkJS.Matrix(1, 0, 0, 1, x, y).concat(cm);
...
{	
	var mtx = new LinkJS.Matrix(1, 0, 0, 1, 0, 0);
	...
	{		
		...
		if(o.rotation%360 != 0)
		{
			var r = o.rotation * LinkJS.DEG_TO_RAD;
			cos = Math.cos(r);
			sin = Math.sin(r);
		}
		...
		mtx.concat(new LinkJS.Matrix(cos*o.scaleX, sin*o.scaleX, -sin*o.scaleY, cos*o.scaleY, o.x, o.y));
	}
	return mtx;
};
...
DisplayObject.prototype.getStage = function()
{
	...
	if(obj instanceof LinkJS.Stage) return obj;
	return null;
};
 ...
 * @return {Object} 显示对象的缓存结果。根据参数toImage不同而返回Canvas或Image对象。
 */
LinkJS.DisplayObject.prototype.cache  = function(toImage, type)
{
	return this._cache = LinkJS.cacheObject(this, toImage, type);
};

/**
 * 清除缓存。
 */
LinkJS.DisplayObject.prototype.uncache = function()
{
	this._cache = null;
};
 ...
 * @param {String} type 指定转换为DataURL格式的图片mime类型。默认为"image/png"。
 */
LinkJS.DisplayObject.prototype.toImage = function(type)
{	
	return LinkJS.cacheObject(this, true, type);
};
...
{
	return LinkJS.UIDUtil.displayObjectToString(this);
};
 ...
 */
var DisplayObjectContainer = LinkJS.DisplayObjectContainer = function(props)
{
	...
	this.id = props.id || LinkJS.UIDUtil.createUID("DisplayObjectContainer");
...
LinkJS.inherit(DisplayObjectContainer, LinkJS.DisplayObject);
...
DisplayObjectContainer.prototype.addChildAt = function(child, index)
{
	...
	if(this.autoSize)
	{		
		var rect = new LinkJS.Rectangle(0, 0, this.rectWidth || this.width, this.rectHeight || this.height);
		var childRect = new LinkJS.Rectangle(child.x, child.y, child.rectWidth || child.width, child.rectHeight || child.height);
 ...
 */
var Stage = LinkJS.Stage = function(props)
{
	this.stageX = 0;
	this.stageY = 0;
	this.paused = false;
	...
	this.id = props.id || LinkJS.UIDUtil.createUID("Stage");
	if(this.context == null) throw "LinkJS.Stage Error: context is required.";
	
	this.updatePosition();
};
LinkJS.inherit(Stage, LinkJS.DisplayObjectContainer);

/**
 * 更新舞台Stage上的所有显示对象。可被LinkJS.Timer对象注册调用。
 */
...
{
	var offset = LinkJS.getElementOffset(this.context.canvas);
 ...
 */
var Bitmap = LinkJS.Bitmap = function(props)
{	
	this.image = null;
	...
	this.id = props.id || LinkJS.UIDUtil.createUID("Bitmap");
...
LinkJS.inherit(Bitmap, LinkJS.DisplayObject);
 ...
 */
var MovieClip = LinkJS.MovieClip = function(props)
{	
	this.interval = 0;	
	this.paused = false;
	...
	this.id = props.id || LinkJS.UIDUtil.createUID("MovieClip");

	if(props.frames) this.addFrame(props.frames);
};
LinkJS.inherit(MovieClip, LinkJS.Bitmap);
 ...
 * @name Button
 * @augments DisplayObjectContainer
 * @class Button类继承自DisplayObjectContainer，是LinkJS中的简单按钮实现。
 * @argument {Object} props 一个对象，包含以下属性：
 * <p>image - Image对象。</p>
 ...
 */
var Button = LinkJS.Button = function(props)
{
	...
	this.id = props.id || LinkJS.UIDUtil.createUID("Button");

	this._skin = new LinkJS.MovieClip({id:"skin", image:props.image});
...
LinkJS.inherit(Button, LinkJS.DisplayObjectContainer);
 ...
 */ 
var Graphics = LinkJS.Graphics = function(props)
{	
	this.lineWidth = 1;
	this.strokeStyle = "0";
	this.lineAlpha = 1;
	...
	this.id = LinkJS.UIDUtil.createUID("Graphics");
	
	this._actions = [];
	this._cache = null;
};
LinkJS.inherit(Graphics, LinkJS.DisplayObject);
 ...
 * <p>var shape = new LinkJS.Graphics({width:500, height:500});</p>
...
{
	var canvas = LinkJS.createDOM("canvas", {width:this.width, height:this.height});
...
{
	var ctx = LinkJS.createDOM("canvas").getContext("2d");
 ...
 */
var Text = LinkJS.Text = function(props)
{
	this.text = "";
	...
	this.id = LinkJS.UIDUtil.createUID("Text");

	if(this.fontMetrics == null) this.fontMetrics = Text.getFontMetrics(this.font);
}
LinkJS.inherit(Text, LinkJS.DisplayObject);
...
{
	if(context instanceof LinkJS.DOMContext)
	{
...
{
	//for DOMContext drawing only
	if(this.drawable == null) this.setDrawable(LinkJS.createDOM("div"), true);
	return this.drawable.get(this, context);
};
...
{
	var metrics = { };
	var elem = LinkJS.createDOM("div", {style:{font:font, position:"absolute"}, innerHTML:"M"});
